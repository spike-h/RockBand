...
struct note {
    ...
    float y;      // y position of the note
...
volatile struct note notes[3][50]; // 3 lanes of notes, 50 is the max number of notes in each lane at a single time (arbitary large number)
...
void draw_notes(int erase)
{
    ...
    for (int i = 0; i < numLanes; i++)
    {
        for (int j = 0; j < activeNotesInLane[i]; j++)
        {
            ...
            {
                fillRect(SCREEN_WIDTH/2 - trackWidth/2 + (i*singleTrackWidth), notes[i][j].y, singleTrackWidth, notes[i][j].height, BLACK);
            }
            else {
                fillRect(SCREEN_WIDTH/2 - trackWidth/2 + (i*singleTrackWidth), notes[i][j].y, singleTrackWidth, notes[i][j].height, notes[i][j].color);
            }
        }
    }
}
...
static PT_THREAD(protothread_spawn_notes(struct pt *pt))
{
    ...
    while (1)
    {
        ...
        PT_YIELD_usec(10000000); // Yield for 100ms
